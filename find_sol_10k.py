...
def do_correction(m, p):
	return m*p + (1-p)*(m + 0.05)

def get_input_matrices(tree, outdir, queries, distances, estimated = '1', correction = '1',  threshold = 0.3):
	tree_obj = read_tree_newick(tree)
	leaves = []
	for d in distances:
		if len(d) == 4 and float(d[2]) > threshold:
			leaves.append(d[0])
	...
	for l in tree_obj.traverse_leaves():
		if l.label not in leaves and l.label not in query_labels:
			parent = l.parent
	...
	true_p = [float(q[1]) for q in queries]
	if estimated == '0':
		true_d = compute_distances(tree_obj, queries)
	else:
		true_d = {d[0]:float(d[3]) for d in distances if d[0] in leaves}
	est_d = {d[0]:float(d[1]) for d in distances if d[0] in leaves}

	if correction == '1':
		# mean_d = np.mean([float(d[1]) for d in distances if len(d) > 2 and float(d[2]) > threshold])
		est_d = {d[0]:do_correction(float(d[1]), float(d[2])) for d in distances if d[0] in leaves}
	else:
		est_d = {d[0]:float(d[1]) for d in distances if d[0] in leaves}
	print(est_d)

	# with open(os.path.join(outdir, "true_mash_distances.txt"), 'w') as f:
	# 	for d in true_d:
	# 		f.write(d + "\t" + str(true_d[d]) + "\n")
	...
	for ql in query_labels:
		...
		if parent.is_root():
			...
			if sibling.edge_length + parent.edge_length == 0:
				true_x.append(0)
			else:
	...
	trued = np.zeros(len(index_to_leaf))
	for i in range(len(index_to_leaf)):
		leaf = index_to_leaf[i]
		d[i] = est_d[leaf]
		trued[i] = true_d[leaf]
	...
	return d, trued, l, C, D, A, index_to_node, node_to_index, index_to_leaf, np.array([node_to_index[correct_anchor[i]] for i in range(k)]), np.array(true_x), np.sum(y * p), np.array(true_p), tree_obj
...
def main():
	...
	with open(os.path.join(args.input_dir, "queries.txt"), 'r') as f:
		lines = f.readlines()
		queries = [l.split() for l in lines]
	k = len(queries)

	with open(os.path.join(args.input_dir, "estimated_distances_pr-h14_dth4.txt"), 'r') as f:
		lines = f.readlines()[1:]
		distances = [l.split() for l in lines]

	
	d, trued, l, C, D, A, index_to_node, node_to_index, index_to_leaf, anchors, true_x, true_y, true_p, pruned_tree = get_input_matrices(tree_obj.newick(), args.outdir, queries, distances, estimated = args.estimated, correction = args.correction)
