...
"""
    ...
    suffix = ''
    strng, label_dict = _nodeprint(tree.root)
    if tree.is_rooted:
        return '[&R] %s%s;' % (strng, suffix), label_dict
    else:
        return '%s%s;' % (strng, suffix), label_dict
...
def _nodeprint(root):
    ...
    for node in root.traverse_postorder():
        label_to_index[node.label] = edge_index
        edge_index += 1
            ...
            for c in node.children:
                ...
                if c.edge_length is not None:
                    if isinstance(c.edge_length, int):
                        ...
                        l_str = str(c.edge_length)
                    out.append(':%s' % l_str)
                out.append('{%d}' % label_to_index[c.label])
                out.append(',')
                del node_to_str[c]
    ...
    return node_to_str[root], label_to_index
